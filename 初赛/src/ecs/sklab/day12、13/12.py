...
X_train,Y_train = data_target_bulid(df_train,N=2)
X_val,Y_val = data_target_bulid(df_validation,N=2)
...
#     X_train,Y_train = data_target_bulid(df_train,N=fold)
#     X_val,Y_val = data_target_bulid(df_validation,N=fold)
...
#     max_score = 0
#     max_alpha = 0
#     for i in [x/10 for x in range(500)]:
#         clf = Lasso(fit_intercept=False,alpha=i)
#         clf.fit(X_train,Y_train)
#         p = clf.predict(X_val)
#         s = score(p,Y_val) 
#         max_score = s if s > max_score else max_score
#         max_alpha = i if s>max_score else max_alpha

#     solution.append({'fold':fold,'max_score':max_score,'max_alpha':max_alpha})
...
clf = Lasso(fit_intercept=False,max_iter=100)
clf.fit(X_train.tolist(),Y_train.tolist())
p = clf.predict(X_val.tolist())
...
# clf = MultiTaskElasticNetCV(cv=10,fit_intercept=False)
# clf.fit(X_train,Y_train)
